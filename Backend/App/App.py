...
def analyze_resume():
    ...
    # Save the file to the Uploaded_Resumes directory
    file_path = os.path.join(UPLOAD_FOLDER, file.filename)
    file.save(file_path)

    # Simulated analysis logic (replace with actual functionality)
    score = 85  # Dummy score
    recommendations = ["Add more technical skills", "Include measurable achievements"]

    # Respond with the analysis result
    return jsonify({"score": score, "recommendations": recommendations})

if __name__ == '__main__':
    app.run(debug=True, port=5000)  # Ensure this port matches the one used in your frontend
